...
std::vector<char> downloader::download_file(const std::string &url, bool show_progress) {
    // Split the URL
    ...
    if (show_progress) JINFO("Started downloading from %s...", url.c_str());
    ...
    while (true) {
        ...
        if (bytes_read > 0) {
            if (show_progress) printf("\rDownloaded %u bytes from %s...", total_read, url.c_str());
        }
    }

    if (show_progress) JINFO("\nFinished downloading %u bytes from %s", total_read, url.c_str());
