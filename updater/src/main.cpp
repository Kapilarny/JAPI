...
void SaveConfig(toml::table &config) {
  std::ofstream config_file("japi/config/updater.cfg",
...
int main() {
  ...
  if (first_run) {
    int auto_update =
        MessageBoxA(NULL, "Do you want to enable auto-updates?",
                    "JojoAPI Updater", MB_YESNO | MB_ICONQUESTION);
  ...
  std::string japi_version_installed =
      ConfigBind(updater_config, "version", "0.0.0");
  ...
  if (first_run) {
    ...
    if (new_hash == 0) {
      JFATAL("Failed to download a new ASBR release! Is this hash correct? (" +
             std::to_string(new_hash) + ")");
      JFATAL("Aborting...");
    ...
    updater_config.insert_or_assign("version",
                                    VersionString(GetLatestJAPIVersion()));
  ...
  if (computed_hash != asbr_hash && !ignore_hashes) {
    JDEBUG(
        "ASBR.exe failed the current checksum! Grabbing the new executable...");

    // Get the new hash
    uint16_t new_hash = DownloadASBR();

    if (new_hash == 0) {
      JERROR("Failed to download a new ASBR release! Is this hash correct? (" +
             std::to_string(new_hash) + ")");
  ...
  if (IsBiggerVersion(latest_version, ParseVersion(japi_version_installed)) &&
      !ignore_hashes) {
